...
GAP funcional: pagina /operativas EN PRODUCCION s63 (`74798ed`, huerfana, gate alumno; 3 trading plans como HTML nativo, PDFs jamas servidos, + hoja de ruta interactiva). Restan: videos + capturas en huecos reservados y enlace de la card (interruptor 1 linea).
```
...
- ⏳ 4 features bloqueantes cerradas — orden s56: go-to session ✅ s56 (prod s57) → session tagging ✅ s57 (prod f851304) → Montecarlo ✅ s58 (prod 0aa8ad3) → card dashboard PDFs/videos AVANZADA s63-bis: página /operativas en producción huérfana `74798ed` (3 trading plans renacidos como HTML nativo —los PDFs jamás se sirven—, hoja de ruta interactiva 6 nodos/11 checks, gate useAuth+NoAccess, pages/operativas.js 483/`95ac7f27`, parches 10-12); restan vídeos + capturas de Ramón en huecos reservados y el enlace de la card (interruptor de 1 línea POST-OK-testers) — último paso antes de apertura
- ✅ Fase 6 trading domain extraída a `lib/trading/` (CUMPLIDO s55 — pricing.js + breach.js + orders.js, smoke producción PASS)
- ✅ Fase 7 reducción `_SessionInner.js` — CERRADA s59 en 1496 líneas: contrato A-G completo, 3 hooks de dominio (useChallengeFlow 429 / usePairData 170 / useTradingActions 245) + 9 componentes/lib extraídos, dashboard 777→620 (bloque Analytics inalcanzable enterrado), openPosition+tpPips muertos eliminados. La diana ≤~1000 presuponía extraer también mountPairRef y el efecto TF: se difirieron por entrelazamiento (DECISIÓN s59) como cortes futuros E2 chart-mount + drag SL/TP. Producción 62c0d9a, 3 checkpoints con smoke, 0 regresiones. Post-F7 (s60): mini-fase admin-wipe en producción 7f80353 (endpoint 5 tablas + simulador_activo=false, confirmación dura por email) + mini-corte H AntimatterLoader en producción 25678d9 (pantalla de carga antimateria + 22 consejos R.A.M.M.FX, corte fino everReadyRef) + saneo BD (FK duplicada fk_session eliminada, DROP backups s45/s48, backup_s57 conservado una sesión más). Post-F7 (s61): poda de 14 imports muertos en _SessionInner 1501→1499 en producción 050ffb4 (criba grep -cw + barrido laxo; realizePnl y Spin absueltos con uso real) + diagnóstico s61-A drawings de posición con borde en horas de finde (raíz verificada en bytes: interpolación lineal del core con el intervalo de las 2 primeras velas + serie sin velas de relleno en el finde; BD sana, daño solo-render; fix gap-aware construido, compilado y APARCADO en fork rammfxtrading-cpu/lightweight-charts-line-tools-core SHA e9a6328 sin llegar a prod — decisión de producto: caso degenerado, no urge) + s61-B feed salta vie ~20:45→dom 22:00 anotado (preexistente) + DROP backup_s57 (gate cumplido, cuadre 154=154 verificado antes y después). Post-F7 (s62): poda de muertos en dashboard.js 620→578 en producción b727488 (fuera import EquityCurve —el componente vive: admin.js lo importa y renderiza—, fuera selectedSession/setSelectedSession con setter jamás invocado tras el Corte G, memo analytics reducido a sus 3 supervivientes closedTrades/wins/totalPnl con deps [trades]) + warning dev borderColor/border muerto en producción 6360978 (raíz en sessionStyles.js: pillPause y togOn pasaban borderColor longhand sobre base con border shorthand; ambos a shorthand completo, visual idéntico) + duplicate key borderRadius muerto en producción 7e2664b (emptyCard del objeto s del dashboard abría y cerraba con borderRadius:12; cae el duplicado). Las 3 deudas dev heredadas del Corte G, muertas.
...
- ✅ Pip-value real por par — COMPLETADO s63, commits 7a15441 (pipValue Opción A: tabla por divisa cotizada USD 10 · JPY 6.25 · CHF 12.65 · CAD 7.20 · GBP 13.35, cierres 5-jun-2026; 11 sitios reales en 6 archivos — los 8 del mapa + 4 del vendor long-short cazados por la regla 8 — harness 0 fails, build, smokes dev+prod PASS; D-question crosses resuelta con tabla estática, exacto-con-feed queda como refinamiento futuro) y 8545445 (presa derivada del smoke: el dibujo long-short pintaba pips x100 en pares JPY, bug preexistente — options.pair no fluía al plugin; pairRef en useDrawingTools, dibujos viejos sin pair siguen x100 hasta editarse o borrarse, asumido pre-lanzamiento). Histórico del fijado s62: calcPnl usaba 10 $/pip/lote fijo para todos los pares (pips*lots*10), exacto solo cuando la divisa cotizada es USD → 7 de los 11 pares de ALL_PAIRS calculan mal (USD/JPY, EUR/JPY, GBP/JPY inflan el P&L ~40-50%; USD/CHF, USD/CAD, AUD/CAD, EUR/GBP valoran el pip en divisa equivocada). El 10 vive hardcodeado en 8 sitios de 5 archivos: pricing.js L14 (calcPnl, fuente) · orders.js L18 (denominador del RR — acoplado: cambiar calcPnl sin él rompe el RR hoy correcto) · breach.js L49+L54 (motor de drawdown del challenge) · PositionOverlay.js L38-39 (P&L proyectado del drawing) · LongShortModal.js L120-121 (lotes por riesgo). Fix: fuente única pipValue(pair) en pricing.js enhebrada en los 5, cambio atómico con harness propio. D-question: los crosses exigen cotización de un segundo par en el instante (EUR/JPY y GBP/JPY → USDJPY; EUR/GBP → GBPUSD; AUD/CAD → USDCAD) que la sesión no carga. Mini-fase s63, primera presa.
- ⏳ Smoke producción exhaustivo multi-par + multi-TF + multi-killzone + Montecarlo + go-to + dashboard + challenge phases

Cuando los 10 criterios sean ✅, abrimos.
