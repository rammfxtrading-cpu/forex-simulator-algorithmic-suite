...
- ⏳ Pip-value real por par — FIJADO s62 como BLOQUEANTE de apertura (decisión Ramón, lo más profesional): calcPnl usa 10 $/pip/lote fijo para todos los pares (pips*lots*10), exacto solo cuando la divisa cotizada es USD → 7 de los 11 pares de ALL_PAIRS calculan mal (USD/JPY, EUR/JPY, GBP/JPY inflan el P&L ~40-50%; USD/CHF, USD/CAD, AUD/CAD, EUR/GBP valoran el pip en divisa equivocada). El 10 vive hardcodeado en 8 sitios de 5 archivos: pricing.js L14 (calcPnl, fuente) · orders.js L18 (denominador del RR — acoplado: cambiar calcPnl sin él rompe el RR hoy correcto) · breach.js L49+L54 (motor de drawdown del challenge) · PositionOverlay.js L38-39 (P&L proyectado del drawing) · LongShortModal.js L120-121 (lotes por riesgo). Fix: fuente única pipValue(pair) en pricing.js enhebrada en los 5, cambio atómico con harness propio. D-question: los crosses exigen cotización de un segundo par en el instante (EUR/JPY y GBP/JPY → USDJPY; EUR/GBP → GBPUSD; AUD/CAD → USDCAD) que la sesión no carga. Mini-fase s63, primera presa.
