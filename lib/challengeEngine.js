...
export function evaluateChallenge({ challengeType, currentPhase, capital, trades, currentTimeIso }) {
  ...
  // worst: el peor ratio (ddDay / capDelDía) histórico indica si quemamos.
  let ddDailyWorstExceededUSD = 0 // cuánto excedió el cap (para detectar fail)
  ...
  for (const day of days) {
    ...
    // Detectar si este día cruzó el cap (para evaluación fail)
    if (ddDay >= dayCapUSD && dayCapUSD > 0) {
      const excess = ddDay - dayCapUSD
      if (excess > ddDailyWorstExceededUSD) ddDailyWorstExceededUSD = excess + 0.01 // marcador > 0
    }
    lastDay = day
  ...
  } else if (ddDailyWorstExceededUSD > 0) {
    // FTMO-exacto: algún día cruzó su cap (startOfDay × 5%).
