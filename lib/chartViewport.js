 ...
 * visible según TF con la tabla _tbars y aplica setVisibleLogicalRange en rAF
 * con isAutoSettingRange flag activada durante la escritura.
 *
 * @param {Object} cr - chartMap.current[pair] entry
 ...
 * Si savedRange no null, restaura en rAF con isAutoSettingRange flag.
 *
 * @param {Object} cr - chartMap.current[pair] entry
 ...
 *   2. Ejecuta applyUpdates() (callback con cr.series.update + bucle phantoms + DEBUG TEMP)
 *   3. Si _rng, restaura en rAF con isAutoSettingRange flag.
 *
 * En el fallback (catch):
 *   1. Regenera 10 phantoms desde cero.
 *   2. Captura _r2 = getVisibleLogicalRange().
 *   3. Hace setData con velas + phantoms regeneradas.
 *   4. Si _r2, restaura en rAF con isAutoSettingRange flag.
 *
 * El callback applyUpdates encapsula código que NO se mueve a esta API
 * (responsabilidad de fase 4 render layer): cr.series.update, bucle phantom,
 * y el bloque [DEBUG TEMP] que sigue inline en _SessionInner.js. El callback
 * captura por closure las variables del scope de updateChart (agg, cr.phantom,
 * tf, _lastT, _tfS2, _mkPhantom).
 *
 * @param {Object} cr - chartMap.current[pair] entry
 * @param {Function} applyUpdates - Callback que ejecuta cr.series.update + phantoms + debug log
 ...
 * @param {Function} fallbackCtx.setSeriesData - Función para actualizar el data layer global
 */
export function restoreOnNewBar(cr, applyUpdates, fallbackCtx) {
  ...
  try {
    ...
    // Fallback: regenerar 10 phantoms desde cero, capturar rango, setData, restaurar
    const { agg, mkPhantom, lastT, tfS2, setSeriesData } = fallbackCtx
 ...
 * se redibujen con el nuevo viewport).
 *
 * @param {Object} cr - chartMap.current[pair] entry
