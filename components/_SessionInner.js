    ...
    <div style={s.root}>
      ...
      <div style={s.topBar}>
        ...
        <div style={{display:'flex',alignItems:'center',gap:6,flexShrink:0}}>
          {/* DEBUG TEMPORAL - QUITAR DESPUÉS DE DIAGNOSTICAR EL BUG DE TRENDLINES CROSS-TF */}
          <button
            style={{...s.fullBtn,background:'rgba(255,200,0,0.15)',border:'1px solid rgba(255,200,0,0.4)',color:'#ffc800',padding:'0 10px',width:'auto',fontSize:11,fontWeight:700}}
            onClick={()=>{
              try {
                const cr = chartMap.current[activePair]
                if (!cr?.chart || !cr?.series) { console.warn('[DEBUG] chart no listo'); return }
                const json = exportTools()
                const tools = JSON.parse(json || '[]')
                const trendlines = tools.filter(t => t.toolType === 'TrendLine')
                if (!trendlines.length) { console.warn('[DEBUG] dibuja una TrendLine antes'); return }

                const tf = pairTf[activePair] || 'H1'
                const data = window.__algSuiteSeriesData
                const realLen = window.__algSuiteRealDataLen
                const timeScale = cr.chart.timeScale()
                const visRange = timeScale.getVisibleLogicalRange()
                const barSpacing = timeScale.options().barSpacing

                console.log('═══════════════════════════════════════════════════')
                console.log('[DEBUG] TF actual:', tf)
                console.log('[DEBUG] data length:', data?.length, '| realLen:', realLen)
                console.log('[DEBUG] barSpacing:', barSpacing)
                console.log('[DEBUG] visibleLogicalRange:', visRange)
                console.log('[DEBUG] dataAtIndex(0):', cr.series.dataByIndex(0, 0))
                console.log('[DEBUG] dataAtIndex(1):', cr.series.dataByIndex(1, 0))
                console.log('[DEBUG] dataAtIndex(realLen-1):', cr.series.dataByIndex(realLen-1, 0))
                console.log('───────────────────────────────────────────────────')

                trendlines.forEach((tl, ti) => {
                  console.log(`[DEBUG] TrendLine #${ti} id=${tl.id}:`)
                  ;(tl.points || []).forEach((p, pi) => {
                    const ts = p.timestamp
                    // Replicamos interpolateLogicalIndexFromTime del plugin
                    const d0 = cr.series.dataByIndex(0, 0)
                    const d1 = cr.series.dataByIndex(1, 0)
                    let logicalIndex = null
                    if (d0 && d1) {
                      const interval = Number(d1.time) - Number(d0.time)
                      if (interval !== 0) {
                        logicalIndex = (Number(ts) - Number(d0.time)) / interval
                      }
                    }
                    const xFromLogical = logicalIndex != null ? timeScale.logicalToCoordinate(logicalIndex) : null
                    const xFromLogicalFloor = logicalIndex != null ? timeScale.logicalToCoordinate(Math.floor(logicalIndex)) : null
                    const xFromTime = timeScale.timeToCoordinate(ts)
                    const yFromPrice = cr.series.priceToCoordinate(p.price)

                    console.log(`  punto ${pi}: ts=${ts} (${new Date(ts*1000).toISOString()}) price=${p.price}`)
                    console.log(`    logicalIndex (float):`, logicalIndex)
                    console.log(`    logicalToCoordinate(float):`, xFromLogical, '← lo que usa el plugin')
                    console.log(`    logicalToCoordinate(floor):`, xFromLogicalFloor)
                    console.log(`    timeToCoordinate(ts):`, xFromTime, '← alternativa')
                    console.log(`    priceToCoordinate:`, yFromPrice)
                  })
                })
                console.log('═══════════════════════════════════════════════════')
                alert('Debug volcado en consola (F12 → Console). Copia todo y pégamelo.')
              } catch(e) { console.error('[DEBUG] error:', e); alert('Error: ' + e.message) }
            }}
            title="DEBUG temporal — diagnóstico anclaje drawings"
          >
            🐛 DEBUG
          </button>
