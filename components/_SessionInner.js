...
export default function SessionPage(){
  ...
  return (
    <div style={s.root}>
      ...
      {tfInput&&(()=>{
        const VALID={'1m':'M1','5m':'M5','15m':'M15','30m':'M30','1h':'H1','4h':'H4','1d':'D1'}
        const match=VALID[tfInput.toLowerCase().trim()]
        const isValid=!!match
        const OPTIONS=[
        ...
        return(
          <div style={{position:'fixed',inset:0,zIndex:9000,display:'flex',alignItems:'center',justifyContent:'center',pointerEvents:'none'}}>
            <div style={{
              ...
              boxShadow:`0 24px 80px rgba(0,0,0,0.5),inset 0 1px 0 rgba(255,255,255,0.25),0 0 0 1px ${isValid?'rgba(41,98,255,0.2)':'rgba(255,255,255,0.05)'}`,
              ...
              <div style={{display:'flex',gap:5,justifyContent:'center',flexWrap:'wrap'}}>
                {OPTIONS.map(o=>{
                  const isCurrentTf=(pairTf[activePair]||'H1')===o.tf
                  const isMatchedTf=match===o.tf
                  return(
                    <div key={o.label} style={{
                      padding:'3px 10px',borderRadius:6,fontSize:10,fontWeight:700,
                      background:isMatchedTf?'rgba(41,98,255,0.35)':isCurrentTf?'rgba(255,255,255,0.1)':'rgba(255,255,255,0.05)',
                      border:`1px solid ${isMatchedTf?'rgba(41,98,255,0.7)':isCurrentTf?'rgba(255,255,255,0.2)':'rgba(255,255,255,0.08)'}`,
                      color:isMatchedTf?'#fff':isCurrentTf?'#fff':'rgba(255,255,255,0.4)',
                    }}>{o.label}</div>
                  )
                })}
              </div>
